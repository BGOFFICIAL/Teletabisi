...
const Register = () => {
 ...
 if (jwt) {
     ...
     if(rolepriv===Employee || rolepriv===Admin || rolepriv===AdminEmployee || rolepriv===User ){
  ...
  const handleCCPasswordChange = (event) => {
    const novaLozinka = event.target.value;
    const isValidCCPassword =
      novaLozinka.length >= 8 &&
      novaLozinka.length <= 20 &&
      novaLozinka.trim() !== "" &&
      novaLozinka === lozinka;
    setCCPassword(novaLozinka);
    setValidated(isValidCCPassword);
  };
...
`}
      ...
      <Form noValidate validated={validated} onSubmit={handleSubmit}>
        ...
        <Row className="justify-content-md-center">
          <Col xs={6}>
            <Form.Group md="3" controlId="validationCustom09">
